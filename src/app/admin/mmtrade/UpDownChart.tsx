...
export function UpDownChart({ slug, points }: { slug: string; points: Pt[] }) {
  ...
  return (
    <div>
      ...
      <svg ref={svgRef} viewBox={`0 0 ${width} ${height}`} className="w-full" onMouseMove={onMove} onMouseLeave={() => setHover(null)}>
        ...
        {hover !== null && <line x1={xs[hover]} y1={pad} x2={xs[hover]} y2={height - pad} stroke="rgba(255,255,255,0.25)" strokeWidth={1} />}
      </svg>
    </div>
  );
}
